...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const TRANSLATIONS = {
        en: {
            ...
            roleTaxi: "Carpool Group",
            roleTaxiSub: "Shared carpool / Public transport",
            taxiBoardTitle: "Shared Carpool Board",
            taxiBoardDesc: "Drag passengers onto carpool groups or click to assign.",
            noActiveTaxis: "No Carpool Groups Created",
            noActiveTaxisDesc: "Click **Add Carpool Group** to set up a carpool group.",
            btnAddTaxi: "Add Carpool Group",
            taxiName: "Carpool",
            editTaxiDetails: "Edit Carpool Details",
            removeTaxi: "Remove Carpool Group",
            taxiLabel: "Carpool",
            lblTaxiSeats: "Carpool Seats Available",
            phTaxiSeats: "4",
            errTaxiSeats: "Must be 1 - 9",
            roleTaxiLabel: "Carpool",
            selfTransportTitle: "Self-Transportation",
            selfTransportDesc: "Own transport / Train / etc.",
            selfTransportCount: "Travelers: {count}",
            noSelfPassengers: "No self-transporting passengers",
        ...
        zh: {
            ...
            roleTaxiSub: "大眾運輸/共乘",
            taxiBoardTitle: "共乘看板",
            taxiBoardDesc: "將乘客拖曳到共乘組上，或點擊車位進行分配。",
            noActiveTaxis: "尚未建立任何共乘組",
            noActiveTaxisDesc: "點擊 **建立共乘組** 開始規劃共乘。",
            btnAddTaxi: "建立共乘組",
            taxiName: "共乘組",
            editTaxiDetails: "編輯共乘資料",
            removeTaxi: "移除此共乘組",
            taxiLabel: "共乘組",
            lblTaxiSeats: "共乘座位數",
            phTaxiSeats: "4",
            ...
            roleTaxiLabel: "共乘組",
            selfTransportTitle: "自行出發/自備交通",
            selfTransportDesc: "自行前往、搭乘大眾運輸等",
            selfTransportCount: "旅客人數: {count}",
            noSelfPassengers: "尚無自行出發的乘客",
    ...
    const fieldsTaxi = document.getElementById('fields-taxi');
    ...
    const DEMO_PARTICIPANTS_EN = [
        ...
        { id: 16, name: 'Carpool Group 1', role: 'taxi', capacity: 4, notes: 'Fare split equally', group: '', licensePlate: '', familyCount: 0, foodBringing: 'no', foodDetails: '', meetLocation: 'QiDu' }
    ];
    ...
    const DEMO_PARTICIPANTS_ZH = [
        ...
        { id: 16, name: '共乘組 1', role: 'taxi', capacity: 4, notes: '車資平分', group: '', licensePlate: '', familyCount: 0, foodBringing: 'no', foodDetails: '', meetLocation: 'QiDu' }
    ];
    ...
    function setupEventListeners() {
        ...
        regRoles.forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                } else if (e.target.value === 'taxi') {
                    fieldsPassenger.classList.add('hidden');
                    fieldsDriver.classList.add('hidden');
                    fieldsTaxi.classList.remove('hidden');
                } else {
        ...
        // Add Taxi Group button inside Taxi Board
        const btnAddTaxi = document.getElementById('btn-add-taxi');
        if (btnAddTaxi) {
            btnAddTaxi.addEventListener('click', () => {
                const taxis = state.participants.filter(p => p.role === 'taxi');
                let nextNum = 1;
                while (taxis.some(t => t.name === `Taxi #${nextNum}` || t.name === `計程車 ${nextNum}` || t.name === `Carpool Group #${nextNum}` || t.name === `共乘組 ${nextNum}`)) {
                    nextNum++;
                }
                const nameVal = currentLang === 'zh' ? `共乘組 ${nextNum}` : `Carpool Group #${nextNum}`;
                const newTaxi = {
                    id: Date.now(),
                    name: nameVal,
                    role: 'taxi',
                ...
                state.participants.push(newTaxi);
                saveState();
                render();
            });
        }
    ...
    function handleFormSubmit(e) {
        ...
        if (regTaxiSeats) regTaxiSeats.classList.remove('invalid');
        ...
        if (roleValue === 'driver') {
            ...
            const taxiSeatsValue = parseInt(taxiSeatsElement.value, 10);
            if (isNaN(taxiSeatsValue) || taxiSeatsValue < 1 || taxiSeatsValue > 9) {
                taxiSeatsElement.classList.add('invalid');
                isValid = false;
            }
            newParticipant.capacity = taxiSeatsValue;
            newParticipant.carModel = currentLang === 'zh' ? '共乘' : 'Carpool';
                ...
                state.participants = state.participants.map(p => {
                    if (p.id === editingId) {
                        ...
                        if (roleValue === 'driver') {
                            ...
                            updated.role = 'taxi';
                            updated.carModel = currentLang === 'zh' ? '共乘' : 'Carpool';
    ...
    function deleteParticipant(id) {
        // Find if this is a driver or taxi
        const item = state.participants.find(p => p.id === id);
        if (item && (item.role === 'driver' || item.role === 'taxi')) {
            ...
            const confirmMsg = isDriver 
                ...
                : `Deleting carpool group "${item.name}" will unassign all their passengers. Continue?`;
            
            const confirmMsgZH = isDriver 
                ? `刪除司機「${item.name}」將會取消分配其所有乘客，是否確定？`
                : `刪除共乘組「${item.name}」將會取消分配其所有乘客，是否確定？`;
    ...
    function sanitizeState() {
        ...
        state.participants = state.participants.map(p => {
            if (p.familyCount !== undefined && p.familyCount !== null) {
                p.familyCount = parseInt(p.familyCount, 10) || 0;
            } else {
                p.familyCount = 0;
            }
    ...
    function render() {
        ...
        drivers.forEach(d => {
            if (d.capacity > 0) {
                let occupied = d.familyCount || 0;
                passengers.forEach(p => {
                    if (p.assignedCarId === d.id) {
                        occupied += 1 + (p.familyCount || 0);
                    }
                });
                totalSeatsOpen += Math.max(0, d.capacity - occupied);
            }
        });
        taxis.forEach(t => {
            if (t.capacity > 0) {
                let occupied = 0;
                passengers.forEach(p => {
                    if (p.assignedCarId === t.id) {
                        occupied += 1 + (p.familyCount || 0);
                    }
                });
                totalSeatsOpen += Math.max(0, t.capacity - occupied);
            }
        });
        ...
        drivers.forEach(d => {
            totalFamilyCount += parseInt(d.familyCount, 10) || 0;
        });
        passengers.forEach(p => {
        ...
        // 3b. Render Taxis Grid
        renderTaxisGrid(taxis, passengers);
    ...
    function renderFoodSummary(participants) {
        ...
        foodBringers.forEach(p => {
            ...
            let roleText = '';
            if (p.role === 'driver') {
                roleText = TRANSLATIONS[currentLang].roleDriverLabel;
            } else if (p.role === 'taxi') {
    ...
    function renderTaxisGrid(taxis, passengers) {
        const taxisGrid = document.getElementById('taxis-grid');
        if (!taxisGrid) return;
        taxisGrid.innerHTML = '';
        ...
        if (taxis.length === 0) {
            taxisGrid.innerHTML = `
                <div class="empty-state-large">
                    <i class="fa-solid fa-taxi animate-float" style="color: var(--accent-amber); font-size: 3rem; margin-bottom: 1rem;"></i>
        ...
        taxis.forEach(t => {
            ...
            taxiCard.className = 'car-card taxi-card';
            taxiCard.dataset.driverId = t.id;
            ...
            if (count === t.capacity) {
                ratioClass = 'ratio-full';
            } else if (count > t.capacity) {
                ratioClass = 'ratio-overflow';
            }

            // Create Seat dots
            let seatsDotsHTML = '';
            for (let i = 0; i < Math.max(t.capacity, count); i++) {
                if (i < count) {
                    const isOverflow = i >= t.capacity;
                    const dotTitle = isOverflow ? TRANSLATIONS[currentLang].overCapacity : TRANSLATIONS[currentLang].seatFilled;
                    seatsDotsHTML += `<span class="seat-dot ${isOverflow ? 'overflow' : 'filled'}" title="${dotTitle}" data-index="${i}"></span>`;
                } else {
                    seatsDotsHTML += `<span class="seat-dot empty" title="${TRANSLATIONS[currentLang].emptySeatClick}" data-index="${i}"></span>`;
                ...
                <div class="car-header">
                    <div class="driver-info">
                        ...
                        <div class="driver-text">
                            ...
                            <span class="car-description">${currentLang === 'zh' ? '共乘小組' : 'Carpool group'}</span>
                            <div class="driver-meta-badges">
            ...
            // Append assigned passenger cards
            const passengerListContainer = taxiCard.querySelector('.car-passengers');
            if (assignedPassengers.length === 0) {
                passengerListContainer.innerHTML = `<p style="font-size: 0.8rem; color: var(--text-muted); text-align: center; padding: 10px; font-style: italic;">${TRANSLATIONS[currentLang].noPassengersAssigned}</p>`;
            } else {
            ...
            // Edit & Delete actions for Taxis
            taxiCard.querySelector('[data-action="edit-taxi"]').addEventListener('click', () => startEdit(t.id));
            taxiCard.querySelector('[data-action="delete-taxi"]').addEventListener('click', () => deleteParticipant(t.id));
    ...
    function exportCSV() {
        ...
        taxis.forEach(t => {
            ...
            const carModelName = currentLang === 'zh' ? '共乘' : 'Carpool';
            if (assigned.length === 0) {
        ...
        if (selfPassengers.length > 0) {
            ...
            csvLines.push(`${TRANSLATIONS[currentLang].csvSelfTransport},,,,,,,,,,,,,,,`);
    ...
    function triggerPrint() {
        ...
        if (drivers.length === 0 && taxis.length === 0) {
        ...
        // Render Self-Transportation in print
        const selfPassengers = passengers.filter(p => p.assignedCarId === 'self');
        if (selfPassengers.length > 0) {
            let totalSelfCount = 0;
            selfPassengers.forEach(p => {
                totalSelfCount += 1 + (p.familyCount || 0);
            });
            printContent.innerHTML += `
                <div class="print-car-group" style="border-left: 3px solid var(--text-muted, #64748b);">
                    <div class="print-car-title">
                        <span>🚶 ${TRANSLATIONS[currentLang].selfTransportTitle}</span>
                        <span>${TRANSLATIONS[currentLang].selfTransportCount.replace('{count}', totalSelfCount)}</span>
                    </div>
                    <ol class="print-passenger-list">
                        ${selfPassengers.map(sp => {
                            const spLoc = sp.meetLocation ? TRANSLATIONS[currentLang][`meetLocation${sp.meetLocation}`] || sp.meetLocation : '';
                            return `
                            <li class="print-passenger-item">
                                <strong>${escapeHTML(sp.name)}</strong> 
                                ${sp.familyCount > 0 ? ` + ${sp.familyCount} ${TRANSLATIONS[currentLang].familyRiding}` : ''}
                                ${sp.group ? `(${escapeHTML(sp.group)})` : ''}
                                [📍${escapeHTML(spLoc)}]
                                ${sp.notes ? `— <em>Notes: ${escapeHTML(sp.notes)}</em>` : ''} 
                                (${TRANSLATIONS[currentLang].foodTitle}: ${getFoodText(sp)})
                            </li>
                            `;
                        }).join('')}
                    </ol>
    ...
    function startEdit(id) {
        ...
        if (p.role === 'driver') {
            ...
            fieldsTaxi.classList.remove('hidden');
            const regTaxiSeats = document.getElementById('reg-taxi-seats');
            if (regTaxiSeats) regTaxiSeats.value = p.capacity !== undefined ? p.capacity : '4';
        } else {
            fieldsPassenger.classList.remove('hidden');
            fieldsDriver.classList.add('hidden');
            fieldsTaxi.classList.add('hidden');
    ...
    function cancelEdit() {
        ...
        // Select passenger role by default and adjust form layout
        const passengerRadio = document.querySelector('input[name="reg-role"][value="passenger"]');
